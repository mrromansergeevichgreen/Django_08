...
Пример рабочего сайта можно увидеть по ссылке [https://kotarba.ru/]
...
Установите следующие зависимости (для Linux):
...
Для Windows необходимо скачать установочный файл с официального сайта и установить postgresql.
...
Перейти в суперпользователя postgresql (для Linux):
```
sudo su - postgres
```
Перейти в командную оболочку postgres (для Linux):
```
psql
```
Перейти в суперпользователя postgresql и сразу командную оболчку (для Windows), после ввода команды введите пароль суперпользователя, заданный вами при установке postgresql:
```
psql - U postgres
```
...
```
Установить для пользователя следующие настройки:
```
...
```
Предоставить пользователю следующие права:
```
